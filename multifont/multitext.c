...
void multitext_writeEx(multitext *const mt, char *const text, unsigned int x, unsigned int y, int plan, int pal, int prio)
{
    ...
    while ((chr = *string++))
    {
        ...
        if (*vrampos == 0)
            GLIBS_MULTITEXT_LOAD_FUNCTION(*vrampos = vrampos_f(tiles), tiles_ptr, pos_in_tileset, chr, tiles);

        GLIBS_MULTITEXT_WRITE_FUNCTION(plan, pal, prio, *vrampos, x, y, width, height);
        x += width;
    }
}
